...
class ScreenController:
    def __init__(self, width, height):
        ...
        self.air_screw = Screw(101, 300)
        self.out_screw = Screw(254, 190)
    ...
    def display(self, surface):
        ...
        # Spin logic
        if self.button_manager.button_bar[1].was_pressed:
            self.air_screw.is_spin = True
        else:
            self.air_screw.is_spin = False

        if self.button_manager.button_bar[2].was_pressed:
            self.out_screw.is_spin = True
        else:
            self.out_screw.is_spin = False
        ...
        if self.button_manager.turn_off.was_pressed:
            ...
            self.button_manager.button_bar[5].add_frame = False

        # Change color of water pipes
        ...
        # MID ELEMENTS
        self.air_screw.display(surface)
        self.out_screw.display(surface)
        ...
        # Close fire when rich 60 degrees
        if self.boxes[2].is_number() and float(self.boxes[2].text) >= 60:
            self.big_fire.is_opened = False
            self.big_fire.was_closed = True
            self.levers[0].rotate(self.levers[0].x + 1, self.levers[0].y + 100)
